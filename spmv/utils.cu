...
int check_answer(dist_matrix_t *mat, const char* filename, data_t* y) {
    int ret, i, m = mat->global_m;
    data_t *ref = (data_t*) malloc(sizeof(data_t) * m * 2);
    data_t *ub = ref + m;
    
    CHECK(ref == NULL, NO_MEM)
    ret = read_vector(mat, filename, "_y.vec", 2, ref);
    CHECK_ERROR(ret, clean(ret, ref))

    for(i = 0; i < m; ++i) {
        if(abs(y[i] - ref[i]) > EPS_TOL * ub[i]) {
            fprintf(stderr, "y[%d] is %e. It should be %e.\nThe error %e exceeds %.0e * %e\n", \
                i, y[i], ref[i], y[i] - ref[i], EPS_TOL, ub[i]);
            return clean(5, ref);
        }
    }
