...
int main(int argc, char **argv) {
    ...
    b0 = memory_alloc(&info);
    b1 = memory_alloc(&info);
    ABORT_IF_NULL(b0)
    ABORT_IF_NULL(b1)


    c0 = memory_alloc(&info);
    c1 = memory_alloc(&info);
    ABORT_IF_NULL(c0)
    ABORT_IF_NULL(c1)
    ...
    if(status == 0) {
        ...
        ABORT_IF_ERROR(read_file(b0, argv[6], &helper, active_procs))
        ABORT_IF_ERROR(read_file(c0, argv[6], &helper, active_procs))
    }
    /* warm up */
    if (type == 7) {
        ans0 = stencil_7(a0, a1,b0, b1,c0, c1, &info, 1);
    } else {
    ...
    start = MPI_Wtime();
    if (type == 7) {
        ans0 = stencil_7(a0, a1,b0, b1,c0, c1, &info, nt);
    } else {
    ...
    if(status == 0) {
        if(argc == 8) {
            ...
            ans1 = (ans0 == a0) ? a1 : a0;
            // ABORT_IF_ERROR(write_file(ans0, argv[7], &helper, active_procs))
        ...
        if(p_id == 0) { 
            double t = end - start; 
            double gflops = 1e-9 * info.global_size_x * info.global_size_y * info.global_size_z * nt * (type * 6 + 9) / t;
...
int write_file(ptr_t grid, const char *file_name, const file_helper_t* h, MPI_Comm active_procs) {
    ...
    MPI_Status status;
    int count;
    int ret = MPI_File_open(active_procs, file_name, MPI_MODE_WRONLY, MPI_INFO_NULL, &input);
    CHECK_ERROR(ret, ret)
    ret = MPI_File_set_view(input, 0, DATA_TYPE, h->file_view, "native", MPI_INFO_NULL);
    CHECK_ERROR(ret, ret)
